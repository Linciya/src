...
<title>DMS Indicator Zone</title>
   ...
   // $("#pvtDist > #pvtTable tr th.pvtRowLabel").closest('#pvtDist > #pvtTable tr th.pvtRowLabel').addClass('selected'); 
	...
	$.ajax({
		...
		success: function(result){
			...
			var data = JSON.parse(result);
			var indPvtInstData=data.indPvtInstData;
			if(typeof indPvtDistData == 'undefined' || indPvtInstData.length === 0)
				{
					$('#pvtInst').empty();
					document.getElementById("noInstData").style.display="inline";
					document.getElementById("SelectedDist").value="";
