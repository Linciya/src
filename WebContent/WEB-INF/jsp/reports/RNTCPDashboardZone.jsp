	  ...
	  var selectedFrequency=document.getElementById("indFrequency");
	  	...
	  	var selectedMonth=document.getElementById("month");
	  	 ...
	  	 var selectedYear=document.getElementById("year");
			 ...
			 $("#pvt1").find('tr:nth-child(2)').css("background", "#00b2ff");
...
<form id="indicatorForm" class="indicatorForm" method="POST" action="rntcpSearchIndicatorList.do">
<table>
		<tr><td>
       ...
       <c:forEach var="frncy" items="${frncyList}">
	   <option value='<c:out value="${frncy}"/>'>${frncy} </option>
			...
			<td><button  id="searchIndicator" class="searchIndicator" onclick="searchIndicatorsByFilter();"><spring:message code="label_searchcriteria_search"/></button></td>	
			</tr>			
	</table>
...
<ul class="tabs-nav">
    <li class=""><a href="#tab-1" rel="nofollow" style="font-size: 12px;height: 30px;line-height: 32px;">${dashboardTabOption}</a>
    </li>
</ul>
...
<div id="dashBoardSecond" class="dashBoardSecond" style="width:50%;height:auto;background:#ffffcc;overflow:auto;margin-left:785px;margin-top:27px;border-color: red;font-size:12px;font-family:Tahoma">
...
<div style="height:21px;background: rgb(135, 206, 250);width:100%;font-size:12px;font-family:Tahoma">
...
$( document ).ready(function() {
	...
	if(indYr<=2015){		
		document.getElementById('indFrequency').value="Quarterly";
		...
		document.getElementById('indFrequency').value="Monthly";
	}
	 ...
	 var selectedIndicator=($(this).closest('tr').children('td:first').text());
	    ...
	    if(selectedIndicator!=null){
	      $.ajax({		
	 		 ...
	 		 async: false,
			 dataType: 'text',
	 		 data:jQuery.param({SelectedIndicatorName:selectedIndicator,Year:year,Month:month,IndCategory:indCategory,Quarter:qtr,DirectorateID:tempDirectorateId}),
...
/* $(document).on("click", "#pvtTable tr th.pvtRowLabel", function(e)  {*/
	...
	$.ajax({		
		 ...
		 async: false,
		 dataType: 'text',
		 data: jQuery.param({ Disrtict: selectedDisrtict, Indicator : indicatorNameTemp,Year:year,Month:month,IndCategory:indCategory,DirectorateID:tempDirectorateId,Quarter:qtr}),		 
		 success: function(result){ 	
...
function onload(){	
	var indYr=${indicatorYer};	
	var indQR="<%=request.getAttribute("indQuarter")%>";
	if(indYr<=2015){
		if(indQR===null || indQR===''){}else{
		document.getElementById('quarter').value=indQR;
		}
		document.getElementById('indFrequency').value="Quarterly";
		document.getElementById('lblmonth').style.display="none";
		document.getElementById('month').style.display="none";
