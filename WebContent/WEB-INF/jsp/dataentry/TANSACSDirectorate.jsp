...
<title>TANSACS Directorate</title>
<%
...
<table>
	...
	<table border="0">
		...
		<td class="directoratenameheader" width="1260px;">
			<label style="padding-left: 320px;font-size: 14px;">Tamil Nadu State AIDS Control society</label><br>
	...
	<tr> 
		...
		<tr>
					...
					<td valign="top" class="serachcriteriasummary" width="220px;" colspan="1">
						...
						<b>Directorate</b> - <label id="gridDirectorateLabel">TANSACS</label> <br><br>&nbsp;
					...
					<td>				
						...
						<div id="tansacsDemographicGrid"  class="scroll" style="padding-top:10px;display: none" ></div>
					</td>
	  </tr>
	<tr>
		...
		<td align="center" width="1260px;">		
			...
			<button id="cancel" type="button" class="btn btn-primary" value="Cancel" onclick="doCancel('<%=request.getContextPath()%>','/tansacsDataEntry.do');">Reset Form</button>&nbsp;
...
<script>
	 ...
	 var demographicIndex;
		 if(isDemographic=='Y'){
		   ...
		   if (e1 != 'undefined') {
			...
			$.ajax({type:"POST",
			url: 'tansacs'+action+'.do',
			'data':  { 
				searchDate: searchDate,month: month,year: year,
				districtId:districtId,institutionType:institutionTypeValue,institutionId:institutionId,
				classificationName:classificationName,frequency:frequency,week:week,quarter:quarter,isDemographic:isDemographic
			    },
			    success: function (output) {
			    	var data = JSON.parse(output);
			    	var gridData = data.aidsDirectorateRecords;
		...
		clearMessage();
		   ...
		   if(formFlag){
			   ...
			   if(formFlag){
				   document.getElementById('submit').disabled=true;
				   $.ajax({type:"POST",
						url: 'tansacs'+submitType+'.do',
						'data':  { 
							handsOnTableData: JSON.stringify(handsOnTableData),searchDate: searchDate,month: month,year: year,
							districtId:districtId,institutionType:institutionTypeValue,institutionId:institutionId,classificationName:classificationName,
 	...
 	function changeDistrict(){
		var form = $('#aidsDirectorateForm');
		var e1 = document.getElementById("districtName");
		var districtId = e1.options[e1.selectedIndex].value;
		$.ajax({type:"POST",
			url: 'tansacsdistrictChange.do',
			'data':  { 
