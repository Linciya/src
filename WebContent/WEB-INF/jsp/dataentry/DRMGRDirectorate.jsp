		...
		<td width="170px;">	
			<select id="department" name="department" class="dropDown" >
				 <c:forEach var="department" items="${departmentList}">
			     	<option value='<c:out value="${department.id}"/>'>${department.name} </option>
			 	</c:forEach>
			</select>
		</td> 
		<td  align="right" width="170px;">
...
<table border="0">
	<tr>
		...
		<td class="directoratenameheader" width="1260px;">
			<label style="padding-left: 320px;font-size: 14px;">TNDr.MGR Medical University </label><br>
	...
	<tr> 
		...
		<b>Directorate</b> - <label id="gridDirectorateLabel">TNDRMGR</label> <br><br>
		<b>Frequency</b> - <label id="gridFrequencyLabel"></label><br><br>
		<label id="gridDepartment"><b>Department - </b> </label> <label id="gridDepartmentLabel"></label><br><br>
		</td>
	...
	<tr>
		...
		<td align="center" width="1260px;">		
			...
			<button id="cancel" type="button" class="btn btn-primary" value="Cancel" onclick="doCancel('<%=request.getContextPath()%>','/drmgrDataEntry.do');">Reset Form</button>&nbsp;
	   ...
	   function doSearch(classificationName,tabId,action){
		   ...
		   if(action=='tabChange'){
			 ...
			 var e3 = document.getElementById("department");
				if (e3 != 'undefined') {
					departmentId = e3.options[e3.selectedIndex].value;
					departmentName = e3.options[e3.selectedIndex].text;
				}	 
			...
			//Since only Quarterly Frequency is required for this Department
			if(departmentName=='Dept of Immunology' && frequency=='Monthly'){
				alert("There is no Indicator Name for Monthly frequency");
				return false;
			}
			...
			url: 'drmgr'+action+'.do',
			'data':  { 
				month: month,year: year,departmentId:departmentId,departmentName:departmentName,classificationName:classificationName,frequency:frequency,quarter:quarter 
			    },
			    success: function (output) {
			    	var data = JSON.parse(output);
			    	var gridData = data.drmgrDirectorateRecords;
			    	...
			    	if(isFrequencySelectionValid){
				    	...
				    	disableFields();
					   	...
					   	document.getElementById('gridDepartment').style.display = "inline";
					   	document.getElementById("gridDepartmentLabel").textContent=departmentName;
		 ...
		 var departmentId;
		   ...
		   if(formFlag){
			   ...
			   if(formFlag){
				   document.getElementById('submit').disabled=true;
				   $.ajax({type:"POST",
						url:'drmgr'+submitType+'.do',
						'data':  { 
							handsOnTableData: JSON.stringify(handsOnTableData),month: month,year: year,departmentId:departmentId,departmentName:departmentName,
  	...
  	function onLoad(){
		...
		document.getElementById('cancel').disabled=true;
	  	document.getElementById("gridFrequencyLabel").textContent='Monthly';
	   	document.getElementById('gridDepartment').style.display = "none";
	}
	</script>
