...
<table>
	...
	<td  align="right" class="required" width="200px;">	 
		...
		<td width="170px;">	
			<select id="system" name="system" class="dropDown" >
				 <!-- <option value="">--Select--</option> -->
              <c:forEach var="system" items="${systemList}">
				     <option value='<c:out value="${system.name}"/>'>${system.name} </option>
				 </c:forEach> 
			</select> 
		</td>
...
<table border="0">
	<tr>
		...
		<td class="directoratenameheader" width="1260px;">
			<label style="padding-left: 320px;font-size: 14px;">Department of Indian Medicine</label><br>
	...
	<tr> 
		...
		<b>Directorate</b> - <label id="gridDirectorateLabel">DIM</label><br><br>
	...
	<tr>
		...
		<td align="center" width="1260px;">		
			...
			<button id="cancel" type="button" class="btn btn-primary" value="Cancel" onclick="doCancel('<%=request.getContextPath()%>','/dimDataEntry.do');">Reset Form</button>&nbsp;
...
<script>
	   ...
	   function doSearch(classificationName,tabId,action){
		   ...
		   if (e1 != 'undefined') {
			...
			if (system != 'undefined' &&  system.options[system.selectedIndex] != 'undefined') {
				systemValue = system.options[system.selectedIndex].text;
			}
			...
			'data':  { 
				...
				districtId:districtId,institutionId:institutionId,institutionType:institutionTypeValue,system:systemValue,classificationName:classificationName,frequency:frequency,week:week,
				quarter:quarter
			    },
			    success: function (output) {
			    	var data = JSON.parse(output);
			    	var gridData = data.dimDirectorateRecords;
	   ...
	   function doSubmit(){
		   ...
		   if(formFlag){
			   ...
			   if(formFlag){
				   document.getElementById('submit').disabled=true;
				   $.ajax({type:"POST",
						url:'dim'+submitType+'.do',
						'data':  { 
							handsOnTableData: JSON.stringify(handsOnTableData),searchDate: searchDate,month: month,year: year,
							districtId:districtId,institutionId:institutionId,institutionType:institutionTypeValue,system:systemValue,
							classificationName:classificationName,frequency:frequency,week:week,quarter:quarter,CSRF_TOKEN:csrfToken
						    },
 	...
 	function changeDistrict(){
		...
		$.ajax({type:"POST",
			url:'dimdistrictChange.do',
			'data':  { 
				districtId:districtId,system:systemValue
			    },
			    success: function (result) {
 	...
 	function changeInstitution(){
		...
		systemValue = system.options[system.selectedIndex].text;
		$.ajax({type:"POST",
			url:'diminstitutionTypeChange.do',
			'data':{ 
				districtId:districtId,institutionType:institutionType,system:systemValue
			    },
			    success: function (result) {
			    	var jsonData = JSON.parse(result);
