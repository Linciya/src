...
<table border="0">
	...
	<tr>
		<td  align="right" width="170px;" class="required">	 
			<b>Program:</b>&nbsp;&nbsp;
		</td>
		<td width="170px;">	
			<select id="programName" class="dropDown" name="programName">
				 <c:forEach var="programName" items="${programList}">
				     <option value='<c:out value="${programName.id}"/>'>${programName.name} </option>
				 </c:forEach>
			</select>
		</td>
		<td  width="170px;">
...
<table border="0">
	<tr>
		...
		<td class="directoratenameheader" width="1260px;">
			<label style="padding-left: 320px;font-size: 14px;">Directorate of Public Health and Preventive Medicine</label><br>
	...
	<tr> 
		...
		<b>Directorate</b> - <label id="gridDirectorateLabel">DPH</label> <br><br>
	...
	<tr>
		...
		<td align="center" width="1260px;">		
			...
			<button id="cancel" type="button" class="btn btn-primary" value="Cancel" onclick="doCancel('<%=request.getContextPath()%>','/dphDataEntry.do');">Reset Form</button>&nbsp;
...
<script>
		...
		function doSearch(classificationName,tabId,action){
		   ...
		   if (e1 != 'undefined') {
			...
			var programName;
			var e4 = document.getElementById("programName");
			if (e4 != 'undefined' &&  e4.options[e4.selectedIndex] != 'undefined') {
				programName = e4.options[e4.selectedIndex].text;
			}			
			...
			url: 'dph'+action+'.do',
			'data':  { 
				searchDate: searchDate,month: month,year: year,	districtId:districtId,institutionType:institutionTypeValue,
				institutionId:institutionId,programName:programName,classificationId:classificationId,classificationName:classificationName,
				frequency:frequency,week:week,quarter:quarter
			    },
			    success: function (output) {
			    	var data = JSON.parse(output);
			    	var gridData = data.dphDirectorateRecords;
	   ...
	   function doSubmit(){
		   ...
		   if(formFlag){
			   ...
			   if(formFlag){
				   document.getElementById('submit').disabled=true;
				   $.ajax({type:"POST",
						url: 'dph'+submitType+'.do',
						'data':  { 
							handsOnTableData: JSON.stringify(handsOnTableData),searchDate: searchDate,month: month,year: year,
							districtId:districtId,institutionId:institutionId,institutionType:institutionTypeValue,programName:programName,classificationId:classificationId,classificationName:classificationName,
