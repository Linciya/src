...
<input id="classificationId"  type="hidden" name="classificationId" value=""/>
...
<table border="0">
	<tr>
		...
		<td class="directoratenameheader" width="1260px;">
			<label style="padding-left: 320px;font-size: 14px;">National Leprosy Eradication Programme</label><br>
			<label>Classification</label>
		</td>
	</tr>	
	<tr> 
		<td valign="top" class="serachcriteriasummary" width="220px;" colspan="1" rowspan="2">
		</td>
		<td width="1260px;" class="categorytab" height="50px;" style="padding-bottom: 20px;">
			<div id="tabs">
			  ...
			  if(categoryList!=null){
				  	...
				  	for(CommonStringList category : categoryList){%>
				  			<li id="tabId<%=tabId%>" role="presentation" style="height:10px;" ><a id="generalCategoryLinkId<%=tabId%>" style="pointer-events:none; font-weight: bold;" href="#" onclick="doSearch('<%= category.getName()%>','<%=tabId%>','tabChange','N');"><%= category.getName()%></a></li> 
				  		<%tabId++;				  		
				  	}
			  }
			  %>
		...
		<tr>
			<td height="20px;" class="demographicheader">
				<div id="demographicTabs">
				  <ul class="nav nav-pills" id="demographicTab">
					 	<li id="nonDemographic" role="presentation" style="height:10px;" ><a id="nonDemographicLinkId" style="pointer-events:none;" href="#" onclick="doDemographicSearch('N');">Non - Demographic</a></li>
					 	<li id="demographic" role="presentation" style="height:10px;" ><a id="demographicLinkId" style="pointer-events:none;" href="#" onclick="doDemographicSearch('Y');">Demographic</a></li>
					</ul>
				</div>
			</td>	
		</tr>
		<tr>					
			<td valign="top" class="serachcriteriasummary" width="220px;" colspan="1">
				...
				<b>Directorate</b> - <label id="gridDirectorateLabel">NLEP</label><br><br>&nbsp;
	...
	<tr>
		...
		<td align="center" width="1260px;">		
			...
			<button id="cancel" type="button" class="btn btn-primary" value="Cancel" onclick="doCancel('<%=request.getContextPath()%>','/nlepDataEntry.do');">Reset Form</button>&nbsp;
...
<script>
	 ...
	 function doSearch(classificationName,tabId,action,isDemographic){
		 var demographicIndex;
		   ...
		   document.getElementById("classificationId").value=tabId;
		   var districtId;
		   var districtName="";
		   var institutionId;
		   var institutionName="";
		   ...
		   if (e1 != 'undefined') {
			...
			url:'nlep'+action+'.do',
			'data':  { 
				searchDate: searchDate,month: month,year: year,districtId:districtId,institutionId:institutionId,classificationName:classificationName,
				frequency:frequency,week:week,quarter:quarter,isDemographic:isDemographic
			    },
			    success: function (output) {
			    	var data = JSON.parse(output);
			    	var gridData = data.nlepDirectorateRecords;
			    	var isDemographicExists = data.isDemographicExists;
			    	...
			    	if(isFrequencySelectionValid){
			    		...
			    		if(isDemographicExists){
			    			document.getElementById("nonDemographicLinkId").style.display = "inline";
			    			document.getElementById("demographicLinkId").style.display = "inline";
			    			document.getElementById("nonDemographicLinkId").style.pointerEvents = "";
			    			document.getElementById("demographicLinkId").style.pointerEvents = "";
			    		}else{
			    			document.getElementById("nonDemographicLinkId").style.display = "none";
			    			document.getElementById("demographicLinkId").style.display = "none";
			    		}
				    	disableFields();
					   	...
					   	document.getElementById("gridFrequencyLabel").textContent=frequency;
							...
							$('#demographicTab li:eq('+demographicIndex+') a').tab('show');
			    	}else{
