...
<input id="selectedWorkshopName"  type="hidden"  value="<%= request.getAttribute("workshopName") %>"/>
<input id="selectedMobileName"  type="hidden" value="<%= request.getAttribute("mobileName") %>"/>
...
<table border="0">
	...
	<tr>
		...
		<td width="170px;">	
			<select id="workshopName" name="workshopName" class="dropDown" onchange="changeWorkshop();">
				<!-- <option value="">--Select--</option> -->
				 <c:forEach var="workshop" items="${workshopList}">
				     <option value='<c:out value="${workshop.id}"/>'>${workshop.name} </option>
				 </c:forEach>
			</select>
		</td> 	
		<td  align="right" width="170px;">	
	...
	<tr>
		...
		<td width="170px;">	
			<select id="mobileName" name="mobileName" class="dropDown">
				<!-- <option value="">--Select--</option> -->
				 <c:forEach var="mobile" items="${mobileList}">
				     <option value='<c:out value="${mobile.id}"/>'>${mobile.name} </option>
				 </c:forEach>
			</select>
		</td> 	
		<td  align="right" width="170px;">
...
<table border="0">
	<tr>
		...
		<td class="directoratenameheader" width="1260px;">
			<label style="padding-left: 320px;font-size: 14px;">State Health Transport Department</label><br>
	...
	<tr> 
		<td valign="top" class="serachcriteriasummary" width="220px;" colspan="1">
			...
			<b>Directorate</b> - <label id="gridDirectorateLabel">SHTD</label> <br><br>
			<b>WorkShop</b> - <label id="gridWorkshopLabel"></label><br><br>
			<b>Mobile WorkShop</b> - <label id="gridMobileLabel"></label><br><br>
	...
	<tr>
		...
		<td align="center" width="1260px;">		
			...
			<button id="cancel" type="button" class="btn btn-primary" value="Cancel" onclick="doCancel('<%=request.getContextPath()%>','/shtdDataEntry.do');">Reset Form</button>&nbsp;
...
<script>
	 ...
	 function doSearch(classificationName,tabId,action){
		   ...
		   var e1 = document.getElementById("workshopName");
		   if (e1 != 'undefined') {
			   workshopId = e1.options[e1.selectedIndex].value;
			   workshopName = e1.options[e1.selectedIndex].text;
		   }
		  var e3 = document.getElementById("mobileName");
			if (e3 != 'undefined') {
				mobileId = e3.options[e3.selectedIndex].value;
				mobileName = e3.options[e3.selectedIndex].text;
			}		
			...
			if(workshopId==""){
				alert("Please select the WorkShop");
				document.getElementById('workshopName').focus();
				return false;
			}
			if(mobileId==""){
				alert("Please select the Mobile WorkShop");
			...
			url: 'shtd'+action+'.do',
			'data':  { 
				searchDate: searchDate,month: month,year: year,workshopId:workshopId,workshopName:workshopName,
				mobileId:mobileId,classificationName:classificationName,frequency:frequency,week:week,
				quarter:quarter
			    },
			    success: function (output) {
			    	var data = JSON.parse(output);
			    	var gridData = data.shtoDirectorateRecords;
			    	...
			    	if(isFrequencySelectionValid){
				    	...
				    	disableFields();
					   	...
					   	document.getElementById("gridWorkshopLabel").textContent=workshopName;
					    document.getElementById("gridMobileLabel").textContent=mobileName;
	   ...
	   function doSubmit(){
		   ...
		   if(formFlag){
			    ...
			    if(formFlag){
				   document.getElementById('submit').disabled=true;
				   $.ajax({type:"POST",
						url: 'shtd'+submitType+'.do',
						'data':  { 
							handsOnTableData: JSON.stringify(handsOnTableData),searchDate: searchDate,month: month,year: year,
							workshopId:workshopId,workshopName:workshopName,mobileId:mobileId,classificationName:classificationName,
							frequency:frequency,week:week,quarter:quarter,CSRF_TOKEN:csrfToken
						    },
 	...
 	function disableFields(){
 		...
 		document.getElementById("workshopName").disabled=true;
 		document.getElementById("mobileName").disabled=true;
 	...
 	function enabledFields(){
 		...
 		document.getElementById("workshopName").disabled=false;
 		document.getElementById("mobileName").disabled=false;
  	...
  	function changeWorkshop(){
		var e1 = document.getElementById("workshopName");
		var workshopId = e1.options[e1.selectedIndex].value;
		var workshopName = e1.options[e1.selectedIndex].text;
		$.ajax({type:"POST",
			url:'shtdworkshopChange.do',
			'data':  { 
				workshopId:workshopId,workshopName:workshopName
			    },
			    success: function (result) {
			    	var jsonData = JSON.parse(result);
			    	var $select = $('#mobileName');                           
 	...
 	function onLoad(){
		...
		document.getElementById('cancel').disabled=true;
	  	document.getElementById("gridWorkshopLabel").textContent='All';
	  	document.getElementById("gridMobileLabel").textContent='All';
