...
<!-- <form name="dmsDirectorateForm" method="POST" action="dmsDirectorateServlet" id="dmsDirectorateForm"> -->
...
<table border="0">
	...
	<tr>
		...
		<td>	
			<select id="hospitalName" name="hospitalName" class="dropDown">
				 <c:forEach var="hospital" items="${hospitalList}">
				     <option value='<c:out value="${hospital.id}"/>'>${hospital.name} </option>
				 </c:forEach>
			</select>
		</td>
		<td  align="right" width="170px;">
...
<table border="0">
	<tr>
		...
		<td class="directoratenameheader" width="1260px;">
			<label style="padding-left: 320px;font-size: 14px;">Directorate of Medical and Rural Health Services</label><br>
	...
	<tr> 
		...
		<b>Directorate</b> - <label id="gridDirectorateLabel">DMS</label> <br><br>
		<b>District</b> - <label id="gridDistrictLabel"></label><br><br>
		<b>Hospital</b> - <label id="gridInstitutionLabel"></label><br><br>
	...
	<tr>
		...
		<td align="center" width="1260px;">		
			...
			<button id="cancel" type="button" class="btn btn-primary" value="Cancel" onclick="doCancel('<%=request.getContextPath()%>','/dmsDataEntry.do');">Reset Form</button>&nbsp;
...
<script>
	...
	function doSearch(classificationName,tabId,action){
		   ...
		   if (typeof e1 != 'undefined') {
			...
			var e2 = document.getElementById("hospitalName");
			if (typeof e2 != 'undefined') {
				hospitalId = e2.options[e2.selectedIndex].value;
				hospitalName=e2.options[e2.selectedIndex].text;
			}		
			...
			$.ajax({			  
				...
				type: "POST",	
				'data': { 
					searchDate: searchDate,month: month,year: year,districtId:districtId,hospitalId:hospitalId,
						classificationName:classificationName,frequency:frequency,week:week,quarter:quarter
			    },
			    success: function (output) {
			    	var data = JSON.parse(output);
			    	var gridData = data.dmsDirectorateRecords;
			    	...
			    	if(isFrequencySelectionValid){
				    	...
				    	disableFields();
					   	...
					   	document.getElementById("gridInstitutionLabel").textContent=hospitalName;
	   ...
	   function doSubmit(){
		   ...
		   if(formFlag){
			   ...
			   if(formFlag){
				   document.getElementById('submit').disabled=true;
				   $.ajax({
						url: 'dms'+submitType+'.do',
						type: "POST",	
						'data':  { 
							handsOnTableData: JSON.stringify(handsOnTableData),searchDate: searchDate,month: month,year: year,
							districtId:districtId,hospitalId:hospitalId,classificationName:classificationName,frequency:frequency,week:week,quarter:quarter,
