...
<html>
	...
	<jsp:include page="MenuLinks.jsp"/>	
		<form name="addLocation" method="POST" action="locationMasterServlet" id="addLocation" class="form-inline">
		...
		<input id="submitType"  type="hidden" name="submitType"/>
			<table border="0">
				<tr>
					<td width=200px; height=40px; colspan="2" align="center" style="background:linear-gradient(to bottom, #007297 49%,#005e7c 100%);">
			        	<label style="font:22px arial;font-weight:bold;">
		       			<font color="white">Add Location</font>
	      				</label>
     			    </td>	 
				</tr>
				<tr>
				...
				<tr>
					<td align="left" class="required rowstyle" width="330px;">
			        	<b>HUD Type</b>
					</td>
					<td style="width: 350px;padding-left: 20px;padding-top:15px;background:#E9FFDB;">
				...
				<tr>
					...
					<td style="width: 350px;padding-left: 20px;padding-top:15px;background:#E9FFDB;">
						<select id="hudName" name="hudName" class="dropDown" title="Select the name of the HUD">
							<option value="">--Select--</option>
				...
				<tr>
					...
					<td style="width: 350px;padding-left: 20px;padding-top:15px;background:#E9FFDB;">
						<input type="text" id="talukName" name="talukName" size="16" title="Enter the name of the Taluk you want to add" />
					</td>
				</tr>
				<tr>
					<td align="left" class="rowstyle" width="210px;">
					...
					<td style="width: 350px;padding-left: 20px;padding-top:15px;background:#E9FFDB;">
						<input type="text" id="blockName" name="blockName" size="16" title="Enter the name of the Block you want to add" />
					</td>
				</tr>
				<tr>
					<td width=140px; style="background:#E9FFDB;" ></td>
					<td width=140px; style="padding-top:15px; padding-bottom:15px;background:#E9FFDB;">&nbsp;&nbsp;&nbsp;
						<button id="submit" type="button" class="btn btn-primary" title="Add new location" onclick="doSubmit();">Submit</button>&nbsp;&nbsp;
						<button id="close" type="button" class="btn btn-primary" title="close Add location page" value="Close" onclick="doClose();">Close</button>&nbsp;&nbsp;
					</td>
				</tr>
			</table>
			</div>
				...
				formFlag = confirm("Do you really want to add the Location?");
				   if(formFlag){
					   document.getElementById('submit').disabled=true;
						$.ajax({type: "POST",
							url: 'LocationMasterAddOnSubmit.do',
							'data':  form.serialize(),
							    success: function (result) {
							    	alert(result);
							    	var directorateId=document.getElementById('directorateId').value;
									window.location.assign("\LocationMasterMaintenance.do?directorateId="+directorateId);
							      },
							      error: function () {
		...
		function changeHudName(){
			...
			$.ajax({type: "POST",
				url: 'locationMasterDirectorateChange.do',
				'data':  { 
		...
		String webUrl =hostName+contextPath+"/"+"LocationMasterMaintenance.do";
		%>
		var webUrl='<%=webUrl%>';
		</script>
	</body>
